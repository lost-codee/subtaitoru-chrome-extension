...
import { SubtitleFetcher } from "../../utils/subtitle-fetcher";
import { parseAssSubtitles, findCurrentSubtitles } from "../../utils/subtitle-parser";
...
const getEpisodeInfo = () => {
  ...
  const subtitleText = document.querySelector('.atvwebplayersdk-subtitle-text')?.textContent;
  if (subtitleText) {
    ...
    if (episodeMatch) {
      ...
      console.log('Found episode info:', { season: seasonNumber, episode: episodeNumber });
      return { 
        episodeNumber,
        seasonNumber,
  ...
  const movieIndicator = document.querySelector('[data-automation-id="runtime-badge"]');
  if (movieIndicator) {
    return { isMovie: true };
  }
...
const AmazonPrimeSubtitles = memo(
  ({ videoElement }: { videoElement: HTMLVideoElement }) => {
    ...
    const [parsedSubtitles, setParsedSubtitles] = useState<ReturnType<typeof parseAssSubtitles>>([]);
    ...
    useEffect(() => {
      const fetchSubtitles = async () => {
        ...
        try {
          ...
          const subtitleResults = await SubtitleFetcher.searchSubtitles(searchParams);
          ...
          const subtitleContent = await SubtitleFetcher.downloadSubtitles(subtitleResults[0]);
          if (!subtitleContent) {
    ...
    useEffect(() => {
      ...
      const updateSubtitles = () => {
        ...
        const tokenizedSubs = currentSubs ? tokenizeJapaneseText(currentSubs) : null;
      ...
      return (
        <div className="flex flex-col items-center p-4 justify-end z-[999] absolute h-full w-full bottom-12">
          <div className="bg-red-100 text-red-800 border border-red-300 p-[12px] rounded-md text-center mb-[8px] animate-[fadeIn] shadow-md">
            <div className="flex items-center text-[14px]">
              <svg className="w-[14px] h-[14px] mr-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
